...
**Updated:** 2026/9/12 18:50:55  
**Exported:** 2026/9/14 6:48:32  
...
9/11/2026, 3:02:05 PM

i want a small change:  
currently single pages (first page and last page) are centered, but odd pages should always be in the right half and even pages should always be in the left half.  
so if there is only one page to display, then instead of the other page there should be a placeholder in the background color.  
this will also fix wrong "fit pages" zoom levels for "landscape pages", where currently the "fit pages" is done for the first page in the old (wrong) single page mode, and when i seek to pages 2 and 3, then the zoom level is wrong, so the left page is cropped on the left edge and the right page is cropped on the right edge

## Response:

9/11/2026, 3:02:10 PM

Yes — that is the right model for a scanned book.

The canvas should always represent **two page slots**:

- **even page → left slot**
- **odd page → right slot**
- if one slot is missing, it is simply painted in the normal background color
- page 1 therefore becomes `[placeholder | page 1]`
- page 2 alone becomes `[page 2 | placeholder]`
- page 2–3 becomes `[page 2 | page 3]`
- final even page becomes `[page N | placeholder]`

Most importantly, **Fit pages now calculates the zoom from the complete two-slot spread**, including the placeholder. That means page 1 and a 2–3 spread use the same geometric model, so landscape scans won't suddenly become cropped when you navigate.

Here is the complete updated script, including the shebang:
...
The key change is `_slot_sizes()`: **Fit pages no longer thinks a single page is the whole spread**. A missing page occupies an invisible background-colored slot, so the geometry is always left-page + right-page.
