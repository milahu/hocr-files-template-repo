# Fix Page Edge Detection

**User:** Anonymous  
**Created:** 2026/9/16 9:35:18  
**Updated:** 2026/9/16 10:19:22  
**Exported:** 2026/9/16 10:19:55  
**Link:** [<https://chatgpt.com/c/6aaa46b6-0250-83ed-9894-33025fda8c49>](https://chatgpt.com/c/6aaa46b6-0250-83ed-9894-33025fda8c49)

## Prompt:

9/16/2026, 9:44:17 AM

help me fix this (mostly ChatGPT-generated) python script: 065-remove-page-borders.py

this script is part of my book scanning pipeline at <https://github.com/milahu/hocr-files-template-repo/blob/main/065-remove-page-borders.py>

the script should extract a book page from a midgray scanner background. currently the script fails when the book page height (config.page_height_mm) is too large. in my failing case: `config.page_height_mm = 215`, but my scanner's maximum page width is `config.max_scan_width_mm = 215.88`. i am scanning the book pages rotated by 90 degrees, so the book page height becomes the scanned image width. the problem here is that if i use too much of my `config.max_scan_width_mm` then the scanner crops one of the page edges, so in the scanned image, the page edge is no longer surrounded by the scanner's midgray background, so i can no longer detect the position and angle of that page edge. but currently, the algorithm in my 065-remove-page-borders.py script requires 3 visible page edges, but in my failing case, there are only 2 visible page edges, so the script throws

    File "065-remove-page-borders.py", line 688, in fit_line_ransac
        raise ValueError("Not enough points for line fit")
    ValueError: Not enough points for line fit

but ideally, the script should fall back to a "use 2 page edges" mode, and it should simply preserve the edge where it cannot detect the page edge
...
9/16/2026, 9:44:19 AM

> **Planning code changes**
>
> I’m inferring that one horizontal page edge is cropped while the other remains visible. I’ll preserve the image boundary as the missing edge and use configured page height to estimate geometry safely.
>
...
Yes. The failure is not really a RANSAC problem; it is a **geometry/model-selection problem**.

Your current `use_three_edge_deskew` path assumes:

- `top_line` exists
- `bottom_line` exists
- `outside_line` exists

But for the 215 mm case, one of `top_pts` / `bottom_pts` can legitimately be empty because the scanner has physically clipped that page edge. In that situation, trying to fit a line to it is the wrong operation.

I looked at the current repository version as well; the file is indeed still structured around the three-edge assumption. ([GitHub](https://github.com/milahu/hocr-files-template-repo/blob/main/065-remove-page-borders.py))

### What I would change

I would **not** make `fit_line_ransac()` silently invent a line. Instead:

1.  Detect which of the three edges are actually available.
2.  Require the outside edge plus **at least one** horizontal edge.
3.  Use the detected horizontal edge + outside edge to determine rotation.
4.  If one horizontal edge is missing, use the **corresponding image boundary as that edge**.
5.  Do **not** try to reconstruct the missing edge from `config.page_height_mm` unless necessary.
6.  This naturally preserves the cropped edge rather than extrapolating page content beyond the scanner image.

That matches your stated desired behavior: *"use 2 page edges" and preserve the edge where it cannot detect the page edge.*

There is one important detail: the missing edge can be either top or bottom, so the code needs to know which one is missing.

Here is the patch I recommend.

\*\*document: \*\*

### 1. Add these helpers after `reject_outliers_vertical()`
...
### 2. Replace the current unconditional RANSAC block
...
### 3. Do not calculate intersections with a missing line

The existing code immediately does:
...
### 4. Do the same for the inside intersections
...
### 5. Fix the rotation calculation

This part currently assumes all three lines exist:
...
This is important: with only two visible edges, the rotation is still fully determined. You have:

- one vertical-ish outside edge
- one horizontal-ish top/bottom edge

So you don't actually need three edges for rotation.

### 6. Most importantly: don't use the missing edge as though it were detected after rotation

After rotation you currently do:
...
This is fine because `inside_top/outside_top/...` now contain image-boundary points when an edge is missing.

However, your perspective transform should distinguish the two cases.
...
and after the `rotated_corners` calculation.

### 7. Do NOT use `config.page_height_mm` to extrapolate the missing edge

This is a subtle but important point.

Your instinct to use `config.page_height_mm = 215` is understandable, but in this particular situation I would **not** use it to manufacture the missing edge.

Your scanner's usable width is:
...
So the missing edge is only ~0.88 mm from the theoretical scan limit. The scanner has already clipped the page.

The information you actually have is:

``` text
image boundary == clipped page edge
```

That is much more reliable than:

``` text
detected edge + 215 mm == hypothetical page edge
```

The latter can actually make the perspective transform generate geometry outside the captured image.

So the fallback should conceptually be:
...
detected top
detected outside
invented bottom 215 mm away
```

### 8. Add a useful debug message

I'd also add this before the perspective transform:
...
That will make the behavior obvious when processing a batch.

### One more bug I would fix

There is also a conceptual problem in this block:
...
If the missing edge is represented by an image boundary, that's okay only if the corresponding top/bottom width is calculated against that boundary. But more importantly, your later code does:
...
and that `x0_top/x1_top` logic is largely redundant because the actual `src` quadrilateral is built directly from:
...
So I would simplify that whole section eventually.

The key geometry is simply:
...
For a missing top edge:

``` text
image boundary
+--------------------------------+
| inside_top -------- outside_top|  <- PRESERVE
|                                |
|                                |
| inside_bottom ----- outside_bottom
+--------------------------------+
...
That is much cleaner than trying to make RANSAC somehow cope with a line that physically isn't present.

**One caveat:** your pasted script contains duplicate definitions of several functions (`ensure_dir`, `save_dbg`, `percentile_threshold`, `keep_largest_component`, `detect_vertical_streaks`, `fit_line_ransac`, etc.). The later definitions shadow the earlier ones. So make the changes to the **second** `fit_line_ransac` definition or, preferably, remove the duplicated block first. Otherwise it's very easy to edit the first function and have Python continue using the second one.

If you want, I can also produce a **clean, directly-patchable diff (`git diff`) for `065-remove-page-borders.py`**, including the 2-edge fallback and the necessary debug changes.

------------------------------------------------------------------------

**Sources:**

- [hocr-files-template-repo/065-remove-page-borders.py at main · milahu/hocr-files-template-repo · GitHub](https://github.com/milahu/hocr-files-template-repo/blob/main/065-remove-page-borders.py)
